...
package com.thd.fido2.fido2_server.mds;

import com.thd.fido2.fido2_server.config.MdsConfig;
import com.thd.fido2.fido2_server.config.MdsInfo;
import com.thd.fido2.fido2_server.exception.MdsV3MetadataException;
import com.thd.fido2.fido2_server.helper.MdsV3MetadataHelper;
import com.thd.fido2.fido2_server.mds.service.MdsProtocolClient;
...
public class MdsFetchTask implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private final MdsV3MetadataHelper mdsV3MetadataHelper;
    ...
    @Autowired
    public MdsFetchTask(MdsConfig mdsConfig, MdsV3MetadataHelper mdsV3MetadataHelper) {
        this.enableMds = mdsConfig.isEnableMds();
        this.mdsV3MetadataHelper = mdsV3MetadataHelper;

        //mdsProtocolClient = new MdsProtocolClient(mdsConfig.getSources().getFirst().getEndpoint());
        if(mdsConfig.getSources() != null){
            mdsProtocolClient = new MdsProtocolClient(mdsConfig.getSources().getFirst().getEndpoint());
        }else {
            mdsProtocolClient = new MdsProtocolClient("http://null");
        }
